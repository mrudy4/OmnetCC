...
void Klient::handleMessage(cMessage *msg)
{
    ...
    if(msg->isSelfMessage())
    {
     ...
     double   czasMiedzyZadaniami=(double) par("czasMiedzyZadaniami");
        ...
        // Jeżeli wiadomość przyszła od serwera, wyślij ją również do statystyk
        if (msg->getArrivalGate()->isName("odSerwera")) {
            // Zmieniamy typ na
            zadanie = check_and_cast<WlasnyMsg *>(msg);
            // Wyślij wiadomość do statystyk
            send(zadanie->dup(), "doStatystyki"); }
        delete msg;
